...
Produces `./dist` with `src/`, `webroot/`, `.env.example` and this README - everything needed, nothing that shouldn't be there (no `.env`, no `data/` contents). Unlike the Node version there's no dependency-install step; PHP files just run.
