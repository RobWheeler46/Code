...
function roleForPack(roleProfileId) {
  return db.prepare(`
    SELECT rp.id, rp.title, rp.grade, rp.role_description, rp.summary, v.version_name
...
async function buildSkfPackDocx({ role, sfiaContext, blocks, meta }) {
  ...
  const overview = [
    ...
    new Paragraph({ heading: HeadingLevel.HEADING_2, children: [new TextRun({ text: 'SFIA context' })] }),
    sfiaContext.length
      ? new Paragraph({ children: [new TextRun({ text: sfiaContext.map(s => `${s.skill_code} L${s.level_number}`).join('  ·  ') })] })
  ...
  const questions = [
    new Paragraph({ children: [new PageBreak()] }),
    new Paragraph({ heading: HeadingLevel.HEADING_1, children: [new TextRun({ text: 'Question set' })] })
  ];
  blocks.forEach((b, i) => questionBlock(b, i + 1).forEach(p => questions.push(p)));
